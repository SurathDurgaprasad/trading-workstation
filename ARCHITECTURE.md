...
| `core/health.py` | The one unified health model — `collect_system_health()` returns a `SystemHealth` (per-component status + one overall HEALTHY/DEGRADED/SAFE_STOP/FAILED verdict), consumed identically by `main.py health`, the dashboard's `/health` route, and `main.py`'s own startup gate (`_run_startup_gate`, wired into `paper-live` and `schedule tick`/`schedule loop`: FAILED refuses to start, SAFE_STOP/DEGRADED warn and continue) | `core/sqlite_util.py`-backed store `integrity_check()`, `llm/provider.py`, `live/state_store.py`, `risk/config.py` |
