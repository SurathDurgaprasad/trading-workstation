...
| 35 | Complete operator dashboard — a new `/intelligence/<symbol>` decision-detail page closes the exact gap this project's own prior operational audit named: decision history with confidence/rationale, full scanner-evidence breakdown, research evidence (news/sector/AI summary), and prediction history (entry/stop/target/outcome/return) — read-only, no market-data fetch or LLM call on page load, reusing every existing store's read methods (`DecisionStore.list_decisions_for_symbol`, `ResearchStore.list_reports_for_symbol`) plus one new, symmetric `PredictionStore.list_predictions_for_symbol` | [`docs/phases/phase-35-complete-operator-dashboard.html`](phases/phase-35-complete-operator-dashboard.html) |
| 36 | Prediction quality & outcome hardening — an audit phase over the existing Phase 23 prediction pipeline (not a feature build). Three real gaps found and fixed: `PredictionRecord` had no structural guard against a degenerate price ordering (now a `model_validator` requiring stop < entry < target); nothing prevented recording the same entry bar twice (`predict`/`shadow-run` accidentally re-run) — now checked via `PredictionStore.has_prediction_for_entry`; and `evaluate_prediction` had no defense against an unadjusted stock split (this project has no split/dividend adjustment source) masquerading as a genuine stop-hit — now detected via a bar-to-bar (not entry-to-bar) implausible-move check, reported as INSUFFICIENT_DATA rather than a fabricated outcome | [`docs/phases/phase-36-prediction-quality-hardening.html`](phases/phase-36-prediction-quality-hardening.html) |
| 37 | Experiment tracking — a new `experiments/` package registers named experiments over the existing, unmodified `DecisionConfig`/`ScannerConfig`/`RiskConfig` version IDs (`python main.py experiment start\|end\|list\|compare`), append-only two-table pattern (status always derived from the latest event, never a mutable field). Comparison isolates each experiment to its OWN `[started_at, ended_at-or-now)` dataset/time boundary AND matching config_version — never a lifetime aggregate — reusing `learning.analysis`'s existing win-rate/return formula unchanged. Real-service verified end-to-end: register → real shadow-run → real comparison correctly attributed a live MSFT BUY prediction to its experiment's window | [`docs/phases/phase-37-experiment-tracking.html`](phases/phase-37-experiment-tracking.html) |
