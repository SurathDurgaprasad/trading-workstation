...
### 29. Unbounded in-memory queue in the Dhan live-feed source (resource exhaustion, disclosed not fixed)

- **FAILURE**: `live/dhan/market_data_source.py::DhanMarketDataSource._bar_queue` is a plain `queue.Queue()` with no `maxsize` -- every completed candle from the WebSocket's own background thread is `put()` onto it unconditionally (`_handle_packet`), and only removed by the consumer calling `next_bar()`. If the consumer falls behind the feed for an extended period -- most plausibly `paper-live`'s own interactive human-approval prompt blocking for a long time while the market keeps ticking -- completed-but-unconsumed `MarketBarEvent`s accumulate on this queue with no upper bound.
- **DETECTION**: found via resource-exhaustion auditing, autonomous hardening cycle 16 (a codebase-wide search for unbounded `list`/`queue`/`dict` growth in every real, in-process-long-lived data structure). Confirmed by direct code reading (`queue.Queue()` at construction, `put()` at the wire-handling call site, no size check anywhere) -- not yet reproduced with an actual sustained-backlog test.
- **SAFE RESPONSE**: none yet -- genuinely unbounded today. Severity assessment: at this project's actual deployment scale (a handful of symbols, minute-or-coarser candles, a single human operator), the realistic backlog size over even a multi-hour approval delay is at most a few thousand small (few-hundred-byte) objects -- a real but currently LOW-URGENCY risk, not an acute one. The risk grows with symbol count, candle granularity, and unattended run duration.
- **RECOVERY**: N/A -- not yet addressed.
- **USER VISIBILITY**: none currently -- no metric or health check surfaces queue depth.
- **Evidence**: **[VERIFIED-CODE]** (direct reading of `live/dhan/market_data_source.py`'s current source), **not** `[VERIFIED-TEST]` -- no test reproduces a sustained backlog.
- **Why not fixed this cycle, disclosed rather than rushed**: the naive fix (`queue.Queue(maxsize=N)`) changes `put()`'s blocking behavior once full -- by default `Queue.put()` blocks the CALLING thread (here, the WebSocket library's own background receive thread) until space frees up, which could itself cause a DIFFERENT failure mode (stalled message processing, missed pings, connection instability) worse than the original unbounded-memory-growth risk. The correct fix needs an explicit, deliberately-chosen backpressure policy (e.g. `put_nowait()` with a drop-oldest-and-log strategy) design AND real-threading verification against this same file's own extensive existing reconnect/concurrency test suite (`tests/test_dhan_market_data_source.py`, 37 tests) to confirm no interaction with the generation-based dedup or the cycle-8 CONNECTING-timeout watchdog -- exactly the same "disclose now, fix in a dedicated future cycle with room for careful verification" judgment this campaign already made once for this same file's CONNECTING-timeout gap (cycle 7 disclosed it, cycle 8 closed it after a structured re-audit).
- **Other resource-exhaustion surfaces checked this cycle, found already bounded (negative audits)**: log growth (`core/logging.py`'s `RotatingFileHandler`, 10MB x 5 backups, already bounded); retry loops (`market_data/resilience.py`'s `RetryPolicy.max_attempts`, the Dhan reconnect's `max_reconnect_attempts`, both already bounded); `live/pipeline.py`'s `pending_approvals` dict (entries are explicitly `del`eted once a decision is reached -- bounded by the naturally-small number of CURRENTLY pending signals, not accumulating forever across a long session); `dashboard/app.py`'s per-request lists and its one persistent module-level cache (`_cached_holidays`, a single value, not a growing collection); `paper/advance.py`'s hold-back/retry mechanism (no internal loop at all -- bounded by the external scheduler-tick cadence that invokes it). Database row growth itself remains a deliberate, already-disclosed accepted limitation (see the closing summary below) -- no automatic retention/archival policy, intentionally, since these rows are exactly the "critical trading state" this project's own rule forbids automatically deleting.

### 30. Concurrent approve/reject of the same pending signal (a narrow, empirically-hard-to-trigger race in a sacred safety file)
...
- **DETECTION**: **executable-tested, autonomous hardening cycle 19** -- closing one of the two items entry #28 (cycle 15) explicitly disclosed as not-yet-concurrency-attacked. A real two-thread test (`tests/test_approval_security.py::test_concurrent_approve_and_reject_of_the_same_signal_never_produces_inconsistent_state`): each thread builds its own fresh `LiveSimPipeline`/`LiveStateStore`/`PaperStore` connections to the SAME temp files (the faithful simulation of two independent dashboard requests), racing `approve_pending` vs `reject_pending` on the identical `signal_id`, synchronized with a `threading.Barrier`.
- **SAFE RESPONSE**: the SAFETY-CRITICAL half of this is already closed by cycle 15's `submit_signal` idempotency fix -- at most ONE `PaperOrder` can ever be created for a given signal_id regardless of how many racing calls reach `submit_signal`, so a genuine duplicate real paper order is already structurally impossible. What remains genuinely unguarded: `LiveStateStore.update_decision` (which records the final APPROVE/REJECT label in `live_state.db`) has NO terminal-state guard at all -- a plain, unconditional `UPDATE`, unlike `paper/store.py`'s own `update_position`/`update_order`'s `WHERE status != <terminal>` CAS pattern. In THEORY, if the reject path's write (fast: one `UPDATE` statement) is delayed by an adversarial thread scheduler long enough to commit AFTER the approve path's write (slower: a full `submit_signal` transaction first), the recorded decision could end up saying REJECTED even though a real PaperOrder was actually created by the approve path -- a misleading audit-trail label, never a duplicated or fabricated trade.
- **RECOVERY**: manual, if it ever occurred -- an operator would need to notice the label/state mismatch via `paper status`/the dashboard's own order list, which always reflects real, ground-truth store state regardless of what `live_state.db`'s decision label says.
- **USER VISIBILITY**: none dedicated today for detecting this specific mismatch, should it occur.
- **Evidence**: **[VERIFIED-TEST]**, empirically strong but not an absolute proof -- the new test passed **10/10** consecutive real-threaded runs. This is not merely luck: the approve path is structurally SLOWER (a full multi-statement `submit_signal` transaction against a separate db file) than the reject path (one `UPDATE` statement), so under any normal thread scheduler the reject write reliably lands and commits well before the approve path even reaches its own `update_decision` call -- naturally biasing the outcome toward safety. This is empirical/structural evidence, not a mathematical guarantee against a pathological scheduler.
- **Why not fixed this cycle, disclosed rather than rushed**: `live/pipeline.py` is one of the 8 live-execution-safety files this campaign treats with the most caution. The correct fix (give `LiveStateStore.update_decision` the same `WHERE`-guarded CAS pattern `paper/store.py` already established for exactly this reason) is small and well-understood, but deserves its own dedicated cycle with room to verify it doesn't interact badly with `expire_pending_approvals`' own unconditional writes or the restart-recovery path -- the same "disclose now, fix in a dedicated future cycle" judgment this campaign has now made three times (the Dhan CONNECTING-timeout gap, cycles 7→8; the `_bar_queue`, entry #29; this one).
