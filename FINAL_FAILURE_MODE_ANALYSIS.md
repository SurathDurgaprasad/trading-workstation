...
- **SAFE RESPONSE**: **partially closed, autonomous hardening cycle 12** -- attacked `scheduler/config.py::ScheduleConfig.from_yaml_file` (an operator-editable, genuinely untrusted-input boundary via `--config`, unlike `core/config.py::Settings`, which has no file/env-var loading at all and was re-confirmed to still have zero reachable untrusted-input path this cycle). Found a REAL, reachable defect: a slot missing a required key, an unparseable time string, an unrecognized action, or a non-mapping slot entry each raised a raw, unwrapped `KeyError`/`TypeError`/`ValueError` -- AND `SchedulerConfigurationError` itself (already used for the pre-existing logical checks: inverted window, non-positive `frequency_minutes`) was missing from `main.py`'s own `_CONTROLLED_ERRORS` tuple, so even the well-designed, clear error message never reached the operator -- both crashed the whole CLI invocation with a full raw traceback. Fixed: every slot-construction failure now raises the same typed `SchedulerConfigurationError` naming the slot's position, and that type is now in `_CONTROLLED_ERRORS`, producing the CLI's standard clean `"Schedule failed: <message>"` surface. **Still open**: `market_data.universe.MarketUniverse.from_yaml_file` (a similar operator-editable config-file boundary) has not yet been audited the same way; `risk/config.py`/`RiskConfig` is already well-guarded by `Field` constraints at every call site (confirmed in cycle 7's numeric-robustness audit) but its own malformed-YAML-file loading path (if any exists) has not been separately re-verified.
- **RECOVERY**: manual (operator fixes the config file).
- **USER VISIBILITY**: now clear and consistent for the scheduler config path (see above); still varies for config paths not yet audited.
- **Evidence**: **[VERIFIED-TEST]** `tests/test_scheduler_config.py` (6 new tests: missing `name`, missing `after`, malformed time string, unrecognized action, non-mapping slot entry, correct-index-naming with multiple slots) and `tests/test_cli_schedule.py::test_a_malformed_schedule_config_file_produces_a_clean_cli_error_not_a_traceback` (a real `main.main()` invocation, `sys.argv` monkeypatched, proving the clean surface end-to-end through the actual CLI entry point, not just the isolated config loader).
