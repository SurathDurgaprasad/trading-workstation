...
- **Not yet audited, disclosed**: `dashboard/app.py`'s approve/reject POST handlers and `live/pipeline.py`'s `approve_pending`/`reject_pending` have not been concurrency-attacked this campaign. `live/pipeline.py` is one of the 8 live-execution-safety files this campaign treats with the most caution -- a change there deserves its own dedicated, carefully-verified cycle rather than a rushed addition alongside unrelated work.
