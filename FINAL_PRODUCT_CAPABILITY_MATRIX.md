...
| Multi-symbol fleet isolation (N independent `paper-live` processes) | **IMPLEMENTED, VERIFIED — new, multi-symbol hardening pass** | `live/runtime_layout.py` (deterministic per-symbol `{runtime-dir}/{SYMBOL}/{paper.db,state.db,predictions.db,logs/}` layout + `verify_no_cross_symbol_contamination` startup guard), `main.py paper-live --runtime-dir`. Chosen deliberately over a single multi-symbol process because `CriticGate` is symbol-bound and `LiveSimPipeline` holds exactly one `critic_gate` object (confirmed by direct code reading before implementation) — `live/pipeline.py` was never modified. Real Dhan instrument-map cache download path also hardened for N-process concurrent startup (atomic write + Windows `os.replace()` retry). See `FINAL_FAILURE_MODE_ANALYSIS.md` entries #50-51. | — |
| Fleet supervision, summary, and shutdown | **IMPLEMENTED, VERIFIED — new, multi-symbol hardening pass** | `live/fleet_supervisor.py` (health detection distinguishing process-death/feed-disconnect/gap-while-alive, bounded per-symbol restarts, `main.py fleet-supervise`), `live/fleet_summary.py` (read-only Symbol/Bars/Fresh/Signals/Trades/Net P&L + TOTAL report, `main.py fleet-summary`), `live/gap_monitor.py` (live `[GAP DETECTED]`/`[GAP ONGOING]` observability closing the real ~15-minute silent gap from the 2026-09-15 session). See `FINAL_FAILURE_MODE_ANALYSIS.md` entries #49, #52. | `fleet-supervise`'s shutdown relies on Windows/POSIX consoles broadcasting Ctrl+C to every child in the same console process group (the common, verified path); its own `terminate()`/`kill()` is a bounded safety net for a worker that does not exit on its own, tested deterministically against fake processes, not a real signal-delivery integration test. |
...
| CLI surface | IMPLEMENTED, VERIFIED | `main.py`, ~30 real subcommands confirmed present this session (`backtest`, `paper`, `live-sim`, `paper-live`, `dashboard`, `scan`, `regime`, `daily-report`, `research`, `decide`, `size`, `predict`, `evaluate`, `learn`, `experiment`, `hypothesis-registry`, `review`, `shadow-run`, `schedule tick/loop/status`, `health`, `readiness-check`, `cache-status`, `universe`), plus two new ones this cycle: `fleet-supervise` and `fleet-summary` (multi-symbol hardening pass — see the new row above) | — |
| Health / readiness | IMPLEMENTED, VERIFIED | `core/health.py`, `main.py health`/`readiness-check`, wired into the startup gate (refuses to start `paper-live`/`schedule tick`/`schedule loop` on a FAILED critical dependency). **Both commands actually run this cycle** (cycle 35): `health` correctly reports DEGRADED (not crashed) with Dhan/Ollama absent, all safety-critical components (`kill_switch`, `risk`, `database`, `disk`) HEALTHY; `readiness-check` correctly reports zero unresolved pending approvals (live confirmation of cycle 26's reconciliation working correctly against this environment's real, lived-in data) and states outright: *"Active strategy: trend_momentum_baseline v1.0 -- SCIENTIFIC VERDICT: NO DEMONSTRATED EDGE"* — the profitability verdict is surfaced by the running tool itself, not only in documentation | — |
| Prediction/outcome tracking (research path: `predict`/`shadow-run`/`evaluate`) | IMPLEMENTED, VERIFIED (mechanism); **INSUFFICIENT DATA** (calibration) | `predictions/direction_forecast.py`, `predictions/tracker.py`, `predictions/store.py`. **Directly queried `data/predictions.db` this cycle** (real local data, not memory): 13 predictions (`KOTAKBANK.NS`×6, `RELIANCE.NS`×4, `DIVISLAB.NS`×1, `GRASIM.NS`×1, `MSFT`×1), entered 2026-09-03/04, 20-bar horizon. Latest evaluation per prediction (as of 2026-09-09): **all 13 still `ACTIVE`, 0–2 of 20 bars observed, zero resolved outcomes.** | Real calibration evidence does not yet exist — the mechanism works, but no prediction has run its full horizon. Cannot support ANY calibration/accuracy claim today. |
| Prediction/outcome tracking (live/paper-live path) | **IMPLEMENTED, VERIFIED** | `live/prediction_recorder.py` bridges `LiveSimPipeline`'s real-time signal generation into the SAME `predictions.db`/`predictions/tracker.py` outcome-resolution engine above, previously never connected to the live path at all. Opt-in via `paper-live --record-predictions`; records every signal (trade AND no-trade) as an immutable `PredictionRecord` with a real, freshly-computed `RiskEngine` snapshot. 8 tests (6 unit, 2 real-CLI end-to-end against cached AAPL data), mutation-tested. See `FINAL_FAILURE_MODE_ANALYSIS.md` entry #40. | Default OFF — existing `paper-live` behavior is unaffected unless explicitly opted in. Real forward-looking accumulation of a live-path prediction cohort has not yet been run (mechanism built and verified, not yet a track record). |
| Automatic outcome resolution for live/paper-live predictions | **IMPLEMENTED, VERIFIED — new this cycle** | Real, empirically-confirmed defect found and closed (Phase C): the live path's default `--interval 1m` meant `python main.py evaluate`'s default `--period 1y` fetched ZERO bars from Yahoo Finance for any recorded prediction (confirmed against LIVE Yahoo data, not assumed — Yahoo caps 1m-granularity requests to ~8 days), silently blocking resolution forever. Fixed with `predictions.tracker.resolution_period_for_interval` (a pure, empirically-derived lookup table; daily-or-longer intervals — the existing research path — are completely unaffected) wired into both existing evaluation call sites (`evaluate`, `shadow-run`), plus new `live.prediction_recorder.evaluate_pending_predictions` called automatically from inside the `paper-live` loop every `--evaluate-every-n-bars` bars (default 20; `0` disables it) so predictions resolve WITHOUT a separate manual `evaluate` invocation. 12 new tests (unit + real-CLI end-to-end against cached AAPL data), mutation-tested twice. See `FINAL_FAILURE_MODE_ANALYSIS.md` entry #41. | Yahoo Finance itself does not retain 1-minute bars older than ~8 days — a 1m prediction left completely unevaluated for longer than that window becomes permanently unresolvable (stuck INSUFFICIENT_DATA, never a false result). Not binding in practice: `horizon_bars=20` at `interval=1m` resolves or expires within 20 minutes of market time. |
...
| Demonstrated economic edge | **NOT_IMPLEMENTED / evidence is negative** | `PromotionVerdict.NEGATIVE` on every fold, for both the model and the deterministic-rule benchmark; canonical source `docs/STRATEGY_EDGE_DISCOVERY_FINAL_OUTPUT.md`. **Surfaced live by the actual `readiness-check` CLI command, run fresh this cycle**: *"Active strategy: trend_momentum_baseline v1.0 -- SCIENTIFIC VERDICT: NO DEMONSTRATED EDGE"* | This is a frozen, settled research verdict, preserved unchanged by explicit standing instruction across every cycle of this campaign — see Section "Profitability Verdict" below |
...
| Finite soak test (this campaign) | IMPLEMENTED, VERIFIED | Cycle 34 — 9,000 ticks / 3 symbols through a real `DhanMarketDataSource` -> `LiveSimPipeline` -> `PaperTradingEngine` -> SQLite chain: no thread leak, bar queue held bounded at 0/200 throughout, DB growth proportional (78 KB), memory allocation proportional to work done (`tracemalloc`) | — |
| Indicator-history buffer bound (`live/pipeline.py::_SymbolBuffer`) | **IMPLEMENTED, VERIFIED — fixed this cycle, was Cycle 34's "accepted limitation"** | `DEFAULT_MAX_BUFFER_BARS = 1000` (a `deque`, O(1) eviction), chosen from a real convergence study against 1254 cached AAPL bars proving indicator values bit-for-bit identical to unbounded history by n=500 (2x margin taken). Benchmarked before/after over 4000 bars: throughput 2.0 bars/s (degrading) -> 7.6 bars/s (flat, stabilized) at the tail, 3.10x faster wall time, 74.4% less peak memory -- a genuine complexity-class fix, not a constant-factor one. 5 new tests, mutation-tested twice. See `FINAL_FAILURE_MODE_ANALYSIS.md` entry #44 | — |
